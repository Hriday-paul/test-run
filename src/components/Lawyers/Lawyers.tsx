...
import { useAllBikesQuery, useAllJobsQuery, useAllLawyersQuery } from "@/redux/api/ads.api"
...
function Lawyers() {
    ...
    if (specilazation) {
        query.specialization = specilazation
    }
    if (searchTerm) {
    ...
    const { isLoading, isError, isSuccess, data } = useAllLawyersQuery(query);
