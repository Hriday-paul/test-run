...
function RentCarDetails({ id }: { id: string }) {
    ...
    return (
        <div >
            ...
            <ShopBanner
                image={bannerimg}
                title="Car Rent Details"
                desc="View Car rent full details"
            >
                <Link href='/' className='text-primary'>Home</Link> <IoIosArrowForward className='' /> Car Rent Details
            </ShopBanner>
            ...
            {(data && isSuccess) ? data?.data?.category !== "CarRent" ? <ErrorComponent /> : <div className='bg-[#F2F4F8] py-8'>
